...
enum State { IDLE, TRANSITION_IN, HIT, TRANSITION_OUT }

# This is a plain frame-driven state machine, not coroutines/await — a
# chain of awaited SceneTreeTimers was the source of the occasional
# freeze (concurrent coroutines racing on the same AnimationPlayer can
# stall in Godot 4). Counting down a float every _process() frame can't
# hang, since nothing here is waiting on an external signal.
...
func _process(delta: float) -> void:
	...
	# Trigger every queued hit whose time has actually arrived. A while
	# loop (not if) in case two hits land in the same frame.
...
func _on_hit_triggered() -> void:
	if state == State.IDLE:
		...
		# Hit has priority: whatever's currently playing (transition in,
		# hit, or transition out) gets cut off immediately in favor of a
		# fresh "hit".
